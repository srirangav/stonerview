...
__private_extern__ osc_t *new_osc_veryrandphaser(int minphaselen, int maxphaselen, int size)
{
	...
	osc->u.overyrandphaser.minphaselen = minphaselen;
	osc->u.overyrandphaser.maxphaselen = maxphaselen;
	osc->u.overyrandphaser.size = size;

	osc->u.overyrandphaser.count = 0;
	/* Pick a random phaselen to start with. */
	osc->u.overyrandphaser.curphaselen = rand_range(minphaselen, maxphaselen);
	/* Pick a random phase to start in. */
	osc->u.overyrandphaser.curphase = rand_range(0, size - 1);
...
__private_extern__ osc_t *new_osc_buffer(osc_t * val)
{
	...
	/*
	 * The last N values are stored in a ring buffer, which we must
	 * initialize here.
	 */
...
__private_extern__ int osc_get(osc_t * osc, int el)
{
	...
	case otyp_VeryRandPhaser:{
			struct overyrandphaser_struct *ox = &(osc->u.overyrandphaser);
			return ox->curphase;
		}
 ...
 * get them all.
 */
__private_extern__ void osc_increment()
{
	osc_t          *osc;
	...
	for (osc = oscroot; osc; osc = osc->next) {
		...
		case otyp_Buffer:{
				...
				/*
				 * We can assume that ox->val has already
				 * been incremented, since it was created
				 * first. This is why new objects are put on
				 * the end of the linked list... yeah, it's
				 * gross.
				 */
				break;
			}
